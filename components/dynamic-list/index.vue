...
<<<<<<< HEAD
	<view class="venue_dynamic_list_container" :style="[_get(config, 'outStyle', {})]">
		<view>
			...
			<load-refresh ref="loadRefresh" :isRefresh="!isIntroductionData && list.length !== 0"
				refreshType="hollowDots" color="#04C4C4" :heightReduce="heightReduce" :fixedHeight="fixedHeight"
				backgroundCover="#F3F5F5" :currentPage="listCurrentPage" :totalPages="listTotalPages"
				...
				<view slot="content-list" class="list_content">
					<view v-for="(item, index) in list">
						...
						<state-item :item="item"
							:itemNavigation="_get(config, 'config.itemNavigation', '/articleDetail/index?id=&title=&type=')"
							v-if="getListItemKey() === 'StateItem'"></state-item>
					</view>
				</view>
			</load-refresh>
		</view>
...
>>>>>>> 9981906ed39b754b2bb5f09af75caf76f3476a0f
	</view>
</template>
...
<<<<<<< HEAD
		...
		methods: {
			...
			updateData() {
				...
				this.listSearch = {
					...searchData,
					..._.get(this.config, 'request.default', {})
				}
				if (_.get(this.config, 'loadApi')) {
					this.fetchList({
						refresh: true
				...
				uni.request({
					...
					complete: (res) => {
						...
						if (['000000', 200].includes(_.get(res, 'data.code'))) {
							...
							this.list = prevList.concat(listField ? _.get(data, listField, []) : _.get(res,
								'list', []))
