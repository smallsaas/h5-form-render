<template>
	<view :class="[`base_signature_container ${param.inputBlock ? 'van_field_input_block' : ''}`]">
		<!-- 旧代码 -->
		<!-- <van-field name="signature" readonly :label="param.label" :required="param.required" :disabled="param.disabled"
			:style="param.style" :error="param.error"
			:error-message="param.error ? param['error-message'] ? param['error-message'] : '' : ''"
			label-class="van_field_label">
			<template slot="input">
				<view class="field_input">
					<button size="mini" type="primary" @click="getSignuteList"
						v-if="!param.readonly&&(userType == 2 && !hideSelect)"
						style="margin-right: 5px;">{{param.value ? '重新选择' : '选择常用签章'}}</button>
					<button size="mini" type="primary" @click="handleSignture" v-if="!param.readonly">
						{{param.value ? '修改' : '填写'}}
					</button>
					<view :style="{
						  backgroundImage: `url(${param.value})`
					  }" class="image" v-if="param.value" style="transform: rotate(-90deg);" @click="handlePreview" />
		...
		<mask v-show="showHistory">
			<view class="history-imageBox" @touchstart="listTouchStart" @touchend="listTouchEnd"
				@touchmove="listTouchMove">
				<view v-for="(item,i) in signlist">
					<image :src="item.url"
						style="background-color: white;border: 1px dotted #555;transform: rotate(-90deg);transform-origin: center center;"
						mode="aspectFit" class="history-image" @click="handleImage(i)"></image>
					<view class="history-name">{{item.name}}</view>
					<view v-show="isImage===i" class="bigImage">
						<image
							style="width: 100vw;height: 100vw;background-color: white;border: 1px solid #555;transform: rotate(-90deg);transform-origin: center center;"
		...
		<mask v-if="showModal">
			<view class="signature_content">
				<view class="content-box">
					<canvas class="mycanvas" canvas-id="signature_canvas" disable-scroll="true"
		...
		<mask v-if="preViewModal">
			<view class="preview_image_content" @click="handleClosePreviewModal">
				<image mode="aspectFit" :src="param.value"
					style="width: 100vw;height: 100vw;background-color: white;border: 1px solid #555;transform: rotate(-90deg);transform-origin: center center;background-color: white;" />
			</view>
		</mask>
	</view>
</template>
...
<script>
	...
	export default {
		...
		created() {
			...
			if (this.param.label === "被督查检查单位现场负责人签名" ) {
