	...
	<view class="base_vants_container" :style="[_get(config, 'outStyle', {})]">
		<van-skeleton row="20" :loading="skeletonLoading">
			<block v-for="(item, index) in fields" :key="index">
				<c-lincense v-if="_get(item,'__config__.tag')==='c-lincense'" @getValue="lincenseValue"></c-lincense>
				<base-vants v-if="_get(item, '__config__.layout') === 'colFormItem'&&getExpect(item)"
					:fields="[{...item}]" :form="form" :Details="Details" :processDefineKey="processDefineKey"
					@change="handleChange" @user="handleUser" @map="handleMap" @clear="handleClear"
					@street="handleStreet" />
				<view v-if="_get(item, '__config__.layout') === 'rowFormItem'"
					...
					<card :title="_get(item, '__config__.componentName')" :Style="{
						...
						<block v-for="(k, i) in _get(item, '__config__.children', [])" :key="i">
							<c-lincense v-if="_get(k,'__config__.tag')==='c-lincense'" @getValue="lincenseValue">
							</c-lincense>
							<base-vants v-if="_get(k, '__config__.layout') === 'colFormItem'&&getExpect(item)"
								:fields="[{...k}]" :form="form" :Details="Details" :processDefineKey="processDefineKey"
								@change="handleChange" @user="handleUser" @map="handleMap" @clear="handleClear"
								@street="handleStreet" />
							<view v-else>
								...
								<base-vants :fields="_get(k, '__config__.children', [])" :form="form" :Details="Details"
									:processDefineKey="processDefineKey" @change="handleChange" @user="handleUser"
									@map="handleMap" @clear="handleClear" @street="handleStreet" />
							</view>
						</block>
					</card>
				</view>
			...
			<view v-if="!Details">
				<view v-if="(workflow||_get(config,'workflow'))&& fields.length > 0">
					<confirm :hideLast="hideLast" :config="ConfirmConfig" v-if="!hideConfirm" :formData="__FORM_DATA__"
						:piId="piId" :LastKey="LastKey" :customValues="customValues" :userlist="userlist||user"
						:jumpUrl="jumpUrl" :processDefineKey="processDefineKey" :noCommit="noCommit"></confirm>
				...
				<view class="button-box" v-if="(!workflow&&!_get(config,'workflow'))&&!hideButton">
					<button class="button" v-if="_get(config, 'LastBtns', false) && fields.length > 0"
						@click="handleLast">
						上一步
					</button>
					<button class="button" v-if="_get(config, 'nextBtns', false) && fields.length > 0"
						@click="handleNext">
						下一步
					</button>

					<button class="button" v-if="_get(config, 'formBtns', true) && fields.length > 0"
						@click="handleSubmit">
						{{_get(config,'formBtnsText',"提交")}}
					</button>
				</view>
				...
				<view v-if="_get(config, 'formCheckBtns', false) && fields.length > 0" class="formcheckbox">
					<view class="messageBox">
						<checkbox color="blue" style="transform:scale(0.8)" @click="handlecheck()" :checked="checks"
							class="Form_CheckBox"></checkbox>
						<span style="color: red;font-size: 10px;">{{_get(config,'formCheckText',"同意协议")}}</span>
					</view>
					<button class="sumbitButton" @click="handleSubmit" :disabled="!checks">
	...
	export default {
		...
		mounted() {
			...
			if (this.processDefineKey && this.config) {
				...
				if (Object.keys(this.config).length > 0) {
					...
					console.log('this.formConfig111111111111 == ', this.formConfig)
				}
			}
		...
		methods: {
			...
			async getKeyFormConfig() {
				...
				let Conf_RES = await LoadComplete({
				...
				const checkRequired = (data = []) => {
					data.map(x => {
						if (x['__vModel__'] === item['__vModel__']) {
							...
							if (['el-upload', 'el-checkbox-group', 'el-multiple-modal-select'].includes(
									formType)) {
			...
			handleRecursive(data = []) {
				data.map(x => {
					if (_.get(x, '__config__.required') === true) {
						...
						if (['el-checkbox-group', 'el-multiple-modal-select'].includes(formType) && _.get(this
								.form, x.__vModel__, []).length === 0) {
							x.error = true
						}
			...
			handleSubmit() {
				...
				if (!this.company) {
					...
					custom = {
						...
						"companyLegalPerson": globalConfig.companyInfo.legalRepresentative || globalConfig.companyInfo
							.personName,
				...
				if (this.ifManualSubmit) {
					...
					if (_.get(this.config, "workflow")) {
						...
						let otherData = {}
						let returnValue = ["reg_num", "person", "phone", "business", "establish_date", "valid_period",
							"captial"
						]
						for (var i in submitData) {
				...
				uni.request({
					...
					complete: (res) => {
						...
						if (_.get(res, 'data.code') === 0) {
							setTimeout(() => {
								if (_.has(this.config, 'submittedNavigation') && this.config
									.submittedNavigation) {
