						...
						uni.getUserInfo({
							...
							success(res) {
								...
								if (_this.userInfo.auth) {
									...
									uni.request({
										// 请求路径
										// url: 'https://zf.smallsaas.cn/auth/oauth/token',//第三方登录
										...
										success(res) {
											...
											if (!res.data.bind) {
												uni.showModal({
													...
													success(button) {
														...
														if (button.confirm) {
															// // console.log(true)
															uni.redirectTo({
																url: "/pages/login/register?query=" +
																	encodeURIComponent(
																		JSON
																		.stringify(
																			list)),
												...
												if (res.data.encryptedData) {
													...
													avatar = JSON.parse(LoginData.rawData)
														.avatarUrl
													// console.log(avatar)
													let query = {
														...
														success(click) {
															...
															if (click.confirm || click
																...
																switch (e) {
																	case '2':
																		uni.reLaunch({
																			url: "/pages/Market-index/Market-index?query=" +
																				encodeURIComponent(
																					JSON
																					.stringify(
																						query
